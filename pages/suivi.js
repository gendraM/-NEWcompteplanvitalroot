...
import { calculerCouvertureSemaine, detecterTrouSuivi } from '../lib/trouSuivi';
import {
...
export default function Suivi() {
  ...
  useEffect(() => {
    ...
    setSuggestionTrouSuivi(detecterTrouSuivi({
      repas: repasSemaine,
  ...
  const handleValiderSemaine = async () => {
    ...
    try {
      ...
      const kcalExtras = repasData.filter(estRepasExtra).reduce((sum, r) => sum + (Number(r.kcal) || 0), 0);
      ...
      const extrasAvecType = repasData.filter(r => estRepasExtra(r) && r.type);

# 🟢 PLAN D'IMPLÉMENTATION — PHASE 5 REPRISE ALIMENTAIRE

**Date de création** : 26/12/2025  
**Branche** : AVANCEMENT-IDEAUX-/TBS-OBJECTIF-POIDS  
**Statut** : ⏳ EN ATTENTE VALIDATION UTILISATEUR

---

## Titre de la tâche  
Implémentation Phase 5 — Alimentation normale contrôlée (Stabilisation durable) avec architecture validée Phases 1-4

---

## **Description précise de la modification attendue**

### Contexte
Les Phases 1, 2, 3 et 4 ont été (ou seront) implémentées avec succès en suivant une architecture éprouvée :
- ✅ Phase 1 : NotificationsPhase1.js + RecettesPhase1Modal.js
- ✅ Phase 2 : NotificationsPhase2.js + RecettesPhase2Modal.js  
- ✅ Phase 3 : NotificationsPhase3.js + RecettesPhase3Modal.js (J6-J10)
- 🔄 Phase 4 : NotificationsPhase4.js + RecettesPhase4Modal.js (J11+, féculents doux)

### Objectif Phase 5
Stabilisation durable avec **alimentation normale contrôlée** (~30% durée reprise) :
- **18 aliments Phase 5** : Pain complet bio, pâtes complètes, lentilles vertes, pois chiches, haricots rouges, fromages (chèvre, comté), yaourt nature, fruits (pomme, poire, kiwi), céréales (sarrasin, épeautre), viandes (bœuf maigre), poissons gras (saumon, maquereau), chocolat noir 85%, noix de cajou
- **Spécificités** : Consolidation acquis, liberté alimentaire progressive, portions normales, 3 repas + collations
- **Notifications horaires** : 8h / 12h30 / 16h / 19h30 (rythme normal)
- **Recettes Cookeo/Marmite** : 8 types (pâtes complètes, lentilles vertes, pois chiches, saumon vapeur, poulet grillé, quinoa complet, légumes rôtis, soupe complète)

### Livrable
- `components/NotificationsPhase5.js` : Affichage des notifications avec rythme alimentaire normal
- `components/RecettesPhase5Modal.js` : Modal avec 8 recettes alimentation équilibrée (toggle Cookeo/Marmite)
- Intégration dans `pages/reprise-alimentaire-apres-jeune.js` (imports, state, composants)

---

## **Fichiers concernés**
- `/components/NotificationsPhase5.js` *(création)*
- `/components/RecettesPhase5Modal.js` *(création)*
- `/pages/reprise-alimentaire-apres-jeune.js` *(modification)*
- `/data/alimentsRepriseJeune.js` *(lecture seule - Phase 5 déjà conforme)*

---

## Etape 1 — **Audit des risques préalable**

### Risques identifiés
1. **Risque technique** : Complexité accrue (18 aliments Phase 5) → Mitigation : Copier architecture Phases 1-4 validée
2. **Risque UX** : Confusion "alimentation normale" = "tout est permis" → Mitigation : Message pédagogique "CONTRÔLÉE" dans notifications
3. **Risque données** : Aliments Phase 5 non conformes → ✅ VÉRIFIÉ : data/alimentsRepriseJeune.js contient 18 aliments Phase 5 valides
4. **Risque régression** : Modification de reprise-alimentaire-apres-jeune.js → Mitigation : Tests complets avant validation
5. **Risque hooks React** : useState/useEffect mal ordonnés → Mitigation : Suivre pattern Phases 1-4
6. **Risque métier** : Portions excessives → Mitigation : Recettes avec portions précises du référentiel

### Ordre des hooks (pattern validé Phases 1-4)
```javascript
// 1️⃣ IMPORTS
import React, { useState, useEffect } from 'react';
import NotificationsPhase1 from '../components/NotificationsPhase1';
import NotificationsPhase2 from '../components/NotificationsPhase2';
import NotificationsPhase3 from '../components/NotificationsPhase3';
import NotificationsPhase4 from '../components/NotificationsPhase4';
import NotificationsPhase5 from '../components/NotificationsPhase5'; // 🆕

// 2️⃣ ÉTAT (dans le composant)
const [modalRecettesPhase5, setModalRecettesPhase5] = useState(false); // 🆕

// 3️⃣ LOGIQUE (après état)
const jourActuel = jours.find(j => j.numero === jourReprise);
const phaseActuelle = jourActuel?.phase;

// 4️⃣ RENDU (après logique)
{phaseActuelle === 5 && (
  <NotificationsPhase5 
    jourNum={jourReprise} 
    onRecettesClick={() => setModalRecettesPhase5(true)} 
  />
)}
```

### Points de vigilance
- ✅ Tous les hooks déclarés en haut du composant fonctionnel
- ✅ Jamais de hooks dans conditions/boucles/maps
- ✅ useState/useEffect AVANT toute logique calculée
- ✅ Handlers définis AVANT le rendu

### Consultation fichier anomalies rollback
✅ **EFFECTUÉ** : Lecture complète du fichier anomalies rollback avant démarrage.
- Aucune anomalie bloquante identifiée pour Phase 5
- Points de vigilance extraits et intégrés dans Etape 6
- Traçabilité totale préservée (aucune suppression dans le fichier)

---

## Etape 2 — **Sous-checklist à valider systématiquement**

### NotificationsPhase5.js
- [ ] `useState` importé de React
- [ ] Variable `horairesPhase5` définie AVANT map
- [ ] Aucun hook dans le map des notifications
- [ ] Message pédagogique "Alimentation normale CONTRÔLÉE" présent
- [ ] Horaires normaux : 8h / 12h30 / 16h / 19h30
- [ ] Prop `onRecettesClick` appelée sur bouton "📖 Voir les recettes"

### RecettesPhase5Modal.js
- [ ] `useState` importé pour toggle Cookeo/Marmite
- [ ] 8 types de recettes : `patesCompletes`, `lentillesVertes`, `poisChiches`, `saumonVapeur`, `pouletGrille`, `quinoaComplet`, `legumesRotis`, `soupeComplete`
- [ ] Chaque recette a 2 versions : Cookeo + Marmite
- [ ] Portions cohérentes avec référentiel (ex: 100g pâtes complètes, 150g saumon)
- [ ] Bouton fermeture modal fonctionnel

### reprise-alimentaire-apres-jeune.js
- [ ] Import `NotificationsPhase5` ajouté en haut
- [ ] Import `RecettesPhase5Modal` ajouté en haut
- [ ] State `modalRecettesPhase5` déclaré avec les autres useState
- [ ] Composant NotificationsPhase5 affiché si `phaseActuelle === 5`
- [ ] Modal RecettesPhase5Modal affiché si `modalRecettesPhase5 === true`

---

## Etape 3 — **Checklist stricte sécurité & qualité**

- [ ] Lecture complète de NotificationsPhase1/2/3/4 pour copier architecture validée
- [ ] Lecture complète de RecettesPhase1/2/3/4Modal pour copier architecture validée
- [ ] Initialisation systématique avant usage (useState en haut)
- [ ] Tous les hooks React déclarés uniquement en haut du corps du composant fonctionnel
- [ ] Séparation stricte : initialisation → logique → handlers → rendu
- [ ] Vérification : toute fonction/handler présent AVANT usage dans rendu
- [ ] Ordre et portée logiques stricts (jamais déclaration prématurée)
- [ ] Pas de doublons ni déclarations superflues
- [ ] Contrôle d'erreur systématique (compilation, runtime, SSR, rendu)
- [ ] Test du rendu sur tous les cas d'usage (différents jours Phase 5)
- [ ] Préservation stricte des fonctionnalités existantes (Phases 1-4 non impactées)
- [ ] Toute anomalie → rollback immédiat + rapport ANOMALIE
- [ ] Documentation claire de chaque étape
- [ ] **Relecture manuelle obligatoire** des déclarations AVANT utilisation
- [ ] **Relecture manuelle ligne par ligne OBLIGATOIRE** : J'ai relu, ligne par ligne et manuellement, la déclaration de tous les useState et useEffect AVANT chaque appel, sans me fier à la mémoire Copilot.
- [ ] **Validation utilisateur OBLIGATOIRE avant implémentation**
- [ ] Toutes les cases ci-dessus cochées et documentées avant de poursuivre

---

## Etape 4 — **Contrôles conformité à réaliser**

### 1️⃣ Lecture fichier anomalies rollback
✅ **EFFECTUÉ** : Aucune anomalie bloquante Phase 5 identifiée dans le fichier rollback.

**⚠️ RAPPEL CRITIQUE** : Aucune suppression ne doit être effectuée sur le fichier rollback lors de l'ajout d'une entrée. Traçabilité totale obligatoire.

### 2️⃣ Checklist de contrôle avant codage
- [ ] Vérifier que data/alimentsRepriseJeune.js contient exactement 18 aliments Phase 5
- [ ] Vérifier que les recettes correspondent aux aliments prioritaires (pâtes, lentilles, saumon, etc.)
- [ ] Vérifier que les horaires sont cohérents avec rythme normal (8h/12h30/16h/19h30)
- [ ] Vérifier que le message "CONTRÔLÉE" est clair dans les notifications
- [ ] Tester l'affichage sur mobile/desktop
- [ ] Tester l'ouverture/fermeture du modal recettes
- [ ] Tester le toggle Cookeo/Marmite dans le modal
- [ ] Vérifier les portions dans les recettes (cohérence référentiel)

### 3️⃣ Analyse de l'audit des risques
✅ Aucune anomalie bloquante. Risques techniques couverts par l'architecture validée Phases 1-4.

### 4️⃣ Gestion anomalies
Si anomalie détectée → Proposition rollback immédiate + Documentation dans fichier ANOMALIE (date/heure) **sans suppression**.

---

## Etape 5 — **Mise à jour de l'avancement**

- [x] Non commencé  
- [ ] En cours  
- [ ] Terminé  

**Avancement précis** : 0 %  
**Historique** :
- 26/12/2025 14:15 — Plan créé, en attente validation utilisateur

---

## Etape 6 — **Point de vigilance**

### Retour d'expérience Phases 1-4
✅ **SUCCÈS CONFIRMÉ** : Architecture NotificationsPhase*.js + RecettesPhase*Modal.js validée sur Phases 1-4.

### Points de vigilance Phase 5
1. **Alimentation "normale CONTRÔLÉE"** : Éviter confusion = tout permis. Message pédagogique clair dans notifications
2. **Diversité alimentaire** : 18 aliments variés (féculents, protéines, fruits, produits laitiers, oléagineux) → recettes équilibrées
3. **Portions normales mais mesurées** : Ex: 100g pâtes, 150g saumon, 120g poulet → afficher dans recettes
4. **Consolidation acquis** : Rappel que Phase 5 = stabilisation, pas fin du protocole
5. **Rythme alimentaire normal** : 3 repas principaux + collations possibles

### Checklist de vérification
- [ ] Message "Alimentation normale CONTRÔLÉE" affiché dans NotificationsPhase5
- [ ] Recettes Cookeo/Marmite cohérentes avec portions data/alimentsRepriseJeune.js
- [ ] Toggle Cookeo/Marmite fonctionnel dans RecettesPhase5Modal
- [ ] Aucune régression sur Phases 1-4 après intégration
- [ ] 8 recettes couvrent diversité alimentaire Phase 5

---

## Etape 7 — **Proposition de rollback**

### Contexte
Si anomalie détectée lors de l'implémentation Phase 5 :

### Action de rollback
1. **Fichier concerné** : `/pages/reprise-alimentaire-apres-jeune.js`
2. **Modification en cause** : Imports NotificationsPhase5 + RecettesPhase5Modal
3. **Alternative sûre** : 
   - Supprimer les imports Phase 5
   - Supprimer le state `modalRecettesPhase5`
   - Supprimer les composants Phase 5 du rendu
   - Revenir à l'état validé Phases 1-4
4. **Traçabilité** : Ajouter entrée dans fichier ANOMALIE rollback avec date/heure/détail

### Documentation anomalie (à remplir si nécessaire)
```
Date : __/__/2025, __h__
Anomalie : [Description précise]
Fichier : [Fichier concerné]
Action : Rollback Phase 5, retour état Phases 1-4 validé
```

**⚠️ AUCUNE SUPPRESSION dans le fichier anomalie, toujours AJOUTER à la suite.**

---

## Etape 8 — **Rapport Markdown Copilot**

### AVANT modification

#### Structure actuelle
```
components/
  ✅ NotificationsPhase1.js
  ✅ NotificationsPhase2.js
  ✅ NotificationsPhase3.js
  🔄 NotificationsPhase4.js (en cours)
  ✅ RecettesPhase1Modal.js
  ✅ RecettesPhase2Modal.js
  ✅ RecettesPhase3Modal.js
  🔄 RecettesPhase4Modal.js (en cours)
  ❌ NotificationsPhase5.js (manquant)
  ❌ RecettesPhase5Modal.js (manquant)

pages/reprise-alimentaire-apres-jeune.js
  ✅ Imports Phases 1-3
  🔄 Imports Phase 4 (en cours)
  ✅ State modalRecettesPhase1/2/3
  🔄 State modalRecettesPhase4 (en cours)
  ✅ Affichage conditionnel Phases 1-3
  🔄 Affichage conditionnel Phase 4 (en cours)
  ❌ Phase 5 non intégrée
```

#### Données Phase 5
- ✅ 18 aliments Phase 5 présents dans data/alimentsRepriseJeune.js
- ✅ Catégories : féculents (5), légumineuses (3), produits laitiers (3), fruits (3), viandes (1), poissons (2), chocolat (1)
- ✅ Spécificité : Alimentation normale CONTRÔLÉE, consolidation acquis

### APRÈS modification (prévision)

#### Structure prévue
```
components/
  ✅ NotificationsPhase1.js
  ✅ NotificationsPhase2.js
  ✅ NotificationsPhase3.js
  ✅ NotificationsPhase4.js
  🆕 NotificationsPhase5.js (création)
  ✅ RecettesPhase1Modal.js
  ✅ RecettesPhase2Modal.js
  ✅ RecettesPhase3Modal.js
  ✅ RecettesPhase4Modal.js
  🆕 RecettesPhase5Modal.js (création)

pages/reprise-alimentaire-apres-jeune.js
  ✅ Imports Phases 1-4
  🆕 Import NotificationsPhase5
  🆕 Import RecettesPhase5Modal
  ✅ State modalRecettesPhase1/2/3/4
  🆕 State modalRecettesPhase5
  ✅ Affichage conditionnel Phases 1-4
  🆕 Affichage conditionnel Phase 5 (si phaseActuelle === 5)
```

#### Fonctionnalités ajoutées
1. **NotificationsPhase5.js** :
   - Horaires : 8h / 12h30 / 16h / 19h30 (rythme normal)
   - Message "Alimentation normale CONTRÔLÉE - Consolidation"
   - Bouton "📖 Voir les recettes"
   
2. **RecettesPhase5Modal.js** :
   - 8 recettes : Pâtes complètes, Lentilles vertes, Pois chiches, Saumon vapeur, Poulet grillé, Quinoa complet, Légumes rôtis, Soupe complète
   - Toggle Cookeo/Marmite pour chaque recette
   - Portions conformes au référentiel (ex: 100g pâtes, 150g saumon)
   
3. **Integration** :
   - Affichage automatique si phase === 5
   - Modal ouverture/fermeture avec state React

### Changements détaillés

#### Initialisation (useState)
```javascript
// AVANT
const [modalRecettesPhase4, setModalRecettesPhase4] = useState(false);

// APRÈS
const [modalRecettesPhase4, setModalRecettesPhase4] = useState(false);
const [modalRecettesPhase5, setModalRecettesPhase5] = useState(false); // 🆕
```

#### Rendu (JSX)
```javascript
// AVANT
{phaseActuelle === 4 && (
  <NotificationsPhase4 jourNum={jourReprise} onRecettesClick={() => setModalRecettesPhase4(true)} />
)}

// APRÈS
{phaseActuelle === 4 && (
  <NotificationsPhase4 jourNum={jourReprise} onRecettesClick={() => setModalRecettesPhase4(true)} />
)}
{phaseActuelle === 5 && (
  <NotificationsPhase5 jourNum={jourReprise} onRecettesClick={() => setModalRecettesPhase5(true)} />
)} // 🆕
```

---

## Etape 9 — **Validation explicite de l'utilisateur (OBLIGATOIRE)**

- [ ] Plan validé par l'utilisateur à la date : ___/___/2025

**⚠️ AUCUNE MODIFICATION DE CODE NE SERA EFFECTUÉE AVANT VALIDATION EXPLICITE DE CE PLAN.**

---

## � Amélioration continue Copilot

- Toujours relier explicitement chaque action utilisateur (ex : clic bouton recettes) à la mise à jour des états métier (ouverture modal)
- **Relecture manuelle obligatoire** à chaque étape : ne pas supposer que la mémoire Copilot suffit, lecture ligne à ligne humaine imposée
- Vérifier systématiquement que chaque étape du plan est traduite en code et testée dans le workflow réel (affichage, activation, réinitialisation, feedback)
- Après chaque modification, tester le parcours complet utilisateur et documenter le résultat (capture, rapport d'exécution)
- Ne jamais supposer qu'un état est synchronisé sans vérification concrète (affichage, console, tests)
- Ajouter un contrôle visuel ou un feedback à chaque action clé pour garantir la conformité UX et métier
- Documenter toute anomalie ou écart dans le fichier dédié et proposer immédiatement une correction ou un rollback (ajout à la fin du fichier, jamais suppression)
- Relire le plan et le template avant chaque implémentation pour s'assurer que toutes les étapes sont respectées
- Se parler à soi-même (Copilot/humain) : « Ai-je bien relié chaque étape du plan au code ? Ai-je testé le workflow complet ? Ai-je documenté chaque action et chaque anomalie ? »

**Rollback automatique (si nécessaire)**
- Inversion immédiate du code (rollback Git)
- Signalement fichier ANOMALIE rollback (date/heure), détail impact, **ajout en fin de fichier uniquement**
- Proposition alternative si risque

**Rapport Markdown Copilot** (date/heure)
- Rapport initial, et rapport après modif, détaillant changements dans chaque section (initialisation, logique, handlers, rendu)
- Toujours relier explicitement chaque action utilisateur à la mise à jour des états métier
- Vérifier systématiquement que chaque étape du plan est traduite en code et testée dans le workflow réel
- Après chaque modification, tester le parcours complet utilisateur et documenter le résultat
- Ne jamais supposer qu'un état est synchronisé sans vérification concrète
- Ajouter un contrôle visuel ou un feedback à chaque action clé
- Documenter toute anomalie ou écart dans le fichier dédié
- **Relecture manuelle systématique, pas de confiance "mémoire IA".**

---

## �📋 **RÉSUMÉ EXÉCUTIF**

### Architecture validée (Phases 1-4)
✅ Pattern éprouvé : NotificationsPhase*.js + RecettesPhase*Modal.js

### Phase 5 — Objectif
Stabilisation durable avec alimentation normale CONTRÔLÉE, 18 aliments variés, 8 recettes équilibrées

### Fichiers à créer
1. `components/NotificationsPhase5.js`
2. `components/RecettesPhase5Modal.js`

### Fichiers à modifier
1. `pages/reprise-alimentaire-apres-jeune.js` (imports + state + rendu)

### Points clés
- 🔑 Copier architecture Phases 1-4 validée
- 🔑 Message "Alimentation normale CONTRÔLÉE" explicite
- 🔑 8 recettes diversifiées avec toggle Cookeo/Marmite
- 🔑 Aucune régression Phases 1-4
- 🔑 Tests complets avant validation
- 🔑 Rythme alimentaire normal (8h/12h30/16h/19h30)

### Spécificités Phase 5
- 🍽️ Alimentation variée : féculents, protéines, fruits, produits laitiers
- 📏 Portions normales mais mesurées
- 🎯 Consolidation acquis du jeûne
- ⚖️ Équilibre durable

### Délai estimé
⏱️ 40-60 minutes (création + intégration + tests)

---

**Prêt pour validation utilisateur** ✅

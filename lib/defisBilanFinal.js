...
export function construireBilanFinalDuree(defi, preuves = {}, maintenant = new Date()) {
  ...
  if (defi.nom === '🍎 Pas de dessert par automatisme') {
    ...
    return bilanGenerique(
      ...
      maintenant,
      dejeuners.length
        ? `Sur les déjeuners saisis pendant ce défi, ${desserts.length} comportaient un dessert identifiable. Ce bilan décrit tes saisies ; il ne suppose pas si le dessert était automatique, choisi ou lié à une occasion.`
  ...
  if (defi.nom === '🧀 1 portion ça suffit') {
    return bilanGenerique(
      ...
      maintenant,
      repas.length
        ? `${repas.length} lignes de repas ont été disponibles pendant le défi. Les quantités saisies ne permettent pas, à elles seules, de conclure combien de portions ont réellement été prises. Le bilan reste donc descriptif.`
  ...
  if (defi.nom === '🚫 Le faux allié') {
    return bilanGenerique(
      ...
      maintenant,
      extras.length
        ? `${extras.length} extra${extras.length > 1 ? 's ont' : ' a'} été saisi${extras.length > 1 ? 's' : ''} pendant la période. Leur présence ne permet pas de conclure qu’il s’agissait d’une compensation : cette intention n’est jamais déduite automatiquement.`
  ...
  if (defi.nom === '🔄 Je brise la chaîne') {
    return bilanGenerique(
      ...
      repas.length || extras.length
        ? 'Tes repas et extras de la période ont été relus pour repérer d’éventuels enchaînements. Sans confirmation utilisateur persistée, le bilan ne transforme jamais une proximité alimentaire en intention ou en échec.'
  ...
  if (defi.nom === '✨ Je me programme du plaisir') {
    ...
    return bilanGenerique(
      ...
      maintenant,
      planifies.length
        ? `${planifies.length} extra${planifies.length > 1 ? 's consommés comportent' : ' consommé comporte'} une indication explicite de planification. Un changement de programme n’est pas considéré comme un échec.`
        : extras.length
  ...
  if (defi.nom === '💧 1 cru par jour') {
    return bilanGenerique(
      ...
      maintenant,
      repas.length
        ? 'Des repas ont été saisis pendant la période, mais les données actuelles ne permettent pas de prouver de façon fiable qu’un aliment était à la fois cru et non sucré. Le bilan reste volontairement prudent.'

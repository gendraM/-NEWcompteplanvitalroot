...
export function construireBilanFinalDuree(defi, preuves = {}, maintenant = new Date()) {
  ...
  const resumeConfirmations = confirmations.length
    ? ` Sur les situations demandant ton avis : ${oui} réponse${oui > 1 ? 's' : ''} « oui », ${non} « non »${incertain ? ` et ${incertain} sans certitude` : ''}. Ces réponses sont conservées avec leur contexte et ne modifient pas la durée du défi.`
    : '';

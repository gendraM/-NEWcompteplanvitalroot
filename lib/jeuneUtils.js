...
export async function sauvegarderListeCoursesReprise(programmeId, userId, listeCourses, options = {}) {
  if (!programmeId || !userId || !Array.isArray(listeCourses)) {
    return { success: false, message: 'Programme, utilisateur ou liste de courses invalide.' };
  }
  const { data, error } = await supabase
    .from('reprises_alimentaires')
    .update({ liste_courses: listeCourses, options, updated_at: new Date().toISOString() })
    .eq('id', programmeId)
    .eq('user_id', userId)
    .select()
    .single();

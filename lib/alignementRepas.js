...
export function calculerScoreAlignementParOccurrence(repasReels = [], repasPlanifiesParType = {}) {
  const occurrences = regrouperRepasReelsParOccurrence(repasReels);
  const alignees = occurrences.filter(({ lignes }) => {
    if (lignes.some(ligne => ligne?.repas_planifie_respecte === true)) return true;
    if (lignes.some(ligne => ligne?.est_extra || ligne?.isFastFood || ligne?.fastFoodType)) return false;

    const type = lignes[0]?.type;
    const plan = type ? repasPlanifiesParType?.[type] : [];
    return classifierAlignementRepas(plan, lignes).statut === STATUTS_ALIGNEMENT_REPAS.ALIGNE;
  });

  return {
    score: occurrences.length > 0
      ? Math.round((alignees.length / occurrences.length) * 100)
      : 0,
    occurrences: occurrences.length,
    alignees: alignees.length
  };
}

 ...
 */
function detecterTypeExtraParKcal(kcal) {
  const valeur = Number(kcal) || 0;
  if (valeur < 150) return 'mini';
  if (valeur <= 280) return 'normal';
  if (valeur <= 500) return 'double';
  return 'majeur';
}

/**
 * Définition métier unique d'un extra.
 *
 * `est_extra` est le champ canonique. Le repli sur `categorie = extra`
 * maintient la compatibilité avec les anciennes saisies qui affichaient bien
 * « extra » mais dont le booléen n'avait pas été synchronisé.
 */
export function estRepasExtra(repas) {
  return repas?.est_extra === true
    || String(repas?.categorie || '').trim().toLowerCase() === 'extra';
}

/**
 * Compte les moments extras, pas les lignes alimentaires.
 * Les lignes d'un repas compose partagent occurrence_repas_id et ne comptent
 * donc qu'une fois. Une ancienne ligne sans occurrence reste un moment propre.
 */
export function compterMomentsExtras(repas = []) {
  if (!Array.isArray(repas)) return 0;

  return new Set(
    repas
      .filter(estRepasExtra)
      .map((repasExtra, index) =>
        repasExtra.occurrence_repas_id || `historique:${repasExtra.id || index}`
      )
  ).size;
}
...
/**
 * Retourne les bornes calendaires lundi → dimanche sans conversion UTC.
 * La date d'un repas reste sa date réelle ; ces bornes servent uniquement
 * à rattacher les lectures et budgets à une même semaine métier.
 */
...
export function calculerExtrasSemaine(weekStart, repasReels) {
  if (!weekStart || !repasReels || !Array.isArray(repasReels)) {
    return { count: 0, kcalTotal: 0, details: [], variation: 0 };
  }
  ...
  try {
    ...
    const extras = repasDesSemaine.filter(estRepasExtra);

    // Plusieurs lignes d'une assiette composée forment un seul moment extra.
    const moments = new Map();
    extras.forEach((extra, index) => {
      const cle = extra.occurrence_repas_id || `historique:${extra.id || index}`;
      const kcal = Number(extra.kcal) || 0;
      const nom = extra.nom || extra.description || extra.aliment || 'Extra';
      const existant = moments.get(cle);
      if (existant) {
        existant.kcal += kcal;
        if (!existant.aliments.includes(nom)) existant.aliments.push(nom);
      } else {
        moments.set(cle, {
          occurrence_repas_id: extra.occurrence_repas_id || null,
          date: extra.date,
          moment: extra.moment || extra.type || 'inconnu',
          aliments: [nom],
          kcal,
        });
      }
    });
    const details = Array.from(moments.values())
      .map(moment => ({
        type: 'extra',
        type_extra: detecterTypeExtraParKcal(moment.kcal),
        nom: moment.aliments.join(' + '),
        aliments: moment.aliments,
        date: moment.date,
        moment: moment.moment,
        kcal: moment.kcal,
        occurrence_repas_id: moment.occurrence_repas_id,
      }))
      .sort((a, b) => `${a.date || ''}-${a.moment || ''}`.localeCompare(`${b.date || ''}-${b.moment || ''}`));
    const kcalTotal = details.reduce((total, extra) => total + extra.kcal, 0);
 ...
 * @param {number} palier - Nombre de moments du palier actuel
 * @returns {string} - Message personnalisé
 */
export function genererMessageFeedback(extrasCount, palier = 5) {
  if (extrasCount === 0) {
    return "Aucun moment extra cette semaine. Tes choix suivent le rythme que tu as créé.";
  }

  if (extrasCount <= palier) {
    return `${extrasCount} moment${extrasCount > 1 ? 's' : ''} extra cette semaine : ton rythme reste dans ton palier actuel.`;
  }

  const difference = extrasCount - palier;
  return `Les extras ont été plus présents cette semaine : ${extrasCount} moments, soit ${difference} au-delà de ton palier. Cette observation t’aide à choisir la suite sans effacer ta progression.`;
}

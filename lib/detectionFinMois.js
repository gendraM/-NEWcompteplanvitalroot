...
function parseDateSafe(dateInput) {
  ...
  if (dateInput instanceof Date) {
    const dateCopy = new Date(dateInput);
    return isNaN(dateCopy.getTime()) ? null : dateCopy;
  }

  if (typeof dateInput === 'string') {
    const isoLocalDateMatch = dateInput.match(/^(\d{4})-(\d{2})-(\d{2})$/);

    if (isoLocalDateMatch) {
      const year = Number(isoLocalDateMatch[1]);
      const month = Number(isoLocalDateMatch[2]);
      const day = Number(isoLocalDateMatch[3]);

      const localNoonDate = new Date(year, month - 1, day, 12, 0, 0, 0);
      return isNaN(localNoonDate.getTime()) ? null : localNoonDate;
    }
  }
...
export function estDerniereValidationDuMois(dateSemaine) {
  ...
  const dateValidation = parseDateSafe(dateSemaine);
...
export function getMoisAnneeValidation(dateSemaine) {
  ...
  const date = parseDateSafe(dateSemaine);

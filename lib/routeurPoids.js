...
export function calculerProfilComplet(profil) {
  ...
  // Conversion taille en cm si nécessaire (si < 10, c'est en mètres)
  const tailleCm = taille < 10 ? taille * 100 : taille;

  // Calcul BMR
  const bmr = calculerBMR(sexe, age, tailleCm, poids_de_depart);
  if (!bmr) return null;

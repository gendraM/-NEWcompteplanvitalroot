...
const getAudioUserId = (audio) => audio?.userId || null;
...
// Sauvegarder audio
export const sauvegarderAudio = async (audioData, userId = null) => {
  try {
...
export const recupererTousLesAudios = async (userId = null) => {
  try {
    const db = await openDB();
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        resolve(userId ? result.filter(audio => getAudioUserId(audio) === userId) : result);
      };
      request.onerror = () => reject(request.error);
    });
  } catch (error) {
...
export const recupererAudio = async (id, userId = null) => {
  try {
    const db = await openDB();
    const transaction = db.transaction([STORE_NAME], 'readonly');
    const store = transaction.objectStore(STORE_NAME);
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const audio = request.result || null;
        if (userId && audio && getAudioUserId(audio) !== userId) {
          resolve(null);
          return;
        }
        resolve(audio);
...
// Supprimer audio
export const supprimerAudio = async (id, userId = null) => {
  try {
    if (userId) {
      const audio = await recupererAudio(id, userId);
...
export const recupererAudiosParType = async (type, userId = null) => {
  try {
    const db = await openDB();
    const transaction = db.transaction([STORE_NAME], 'readonly');
    const store = transaction.objectStore(STORE_NAME);
...
export const calculerEspaceUtilise = async (userId = null) => {
  try {
    const audios = await recupererTousLesAudios(userId);
...
export const nettoyerVieuxAudios = async (limite = 50, userId = null) => {
  try {
    const audios = await recupererTousLesAudios(userId);
...
export const exporterAudio = async (id, userId = null) => {
  try {
    const audio = await recupererAudio(id, userId);
    if (!audio) return false;
...
export const recupererAudiosParJeune = async (dateDebut, dateFin, userId = null) => {
  try {
    const audios = await recupererTousLesAudios(userId);
...
export const archiverAudiosJeune = async (dateDebut, dateFin, idJeune, userId = null) => {
  try {
    const audios = await recupererAudiosParJeune(dateDebut, dateFin, userId);
    ...
    const archivesExistantes = JSON.parse(localStorage.getItem(userId ? `audiosArchives_${userId}` : 'audiosArchives') || '{}');
    ...
    localStorage.setItem(userId ? `audiosArchives_${userId}` : 'audiosArchives', JSON.stringify(archivesExistantes));
...
export const recupererAudiosArchives = async (idJeune, userId = null) => {
  try {
    const archives = JSON.parse(localStorage.getItem(userId ? `audiosArchives_${userId}` : 'audiosArchives') || '{}');

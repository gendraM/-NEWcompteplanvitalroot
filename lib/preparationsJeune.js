...
export async function getHistoriquePreparationsJeuneSupabase(userId) {
	...
	try {
		...
		return (data || []).map(transformToCamelCaseFromPreparation);
	} catch (e) {
...
// Déplacer une préparation dans la corbeille cloud sans supprimer ses données.
export async function archiverPreparationJeuneSupabase(userId, id) {
...
// Restaurer une préparation placée dans la corbeille cloud.
export async function restaurerPreparationJeuneSupabase(userId, id) {
	if (!userId || !id) return false;
	const { error } = await supabase
		.from('preparations_jeune')

import { normaliserRepasPlanifie, trouverAlimentReferentiel } from './planificationRepas';
...
function construireIngredient(ligne, referentiel) {
  ...
  const reference = trouverAlimentReferentiel(referentiel, ligne?.aliment);
  const qnReference = Number(reference?.qn);
  return {
    id: ligne?.id || null,
    aliment: String(ligne?.aliment || '').trim() || 'Aliment non renseigné',
    categorie: String(ligne?.categorie || reference?.categorie || '').trim() || null,
    qn: Number.isFinite(qnReference) && qnReference >= 1 && qnReference <= 5 ? qnReference : null,

...
export const DELAI_NOUVELLE_PROPOSITION_JOURS = 2;
...
export async function nouvellePropositionEnPause() {
  const userId = await utilisateurCourant();
  const { data, error } = await supabase
    .from('defis_solicitations')
  ...
  if (error) throw error;
  if (!data?.last_response_at) return false;

  const derniereReponse = new Date(data.last_response_at).getTime();
  const reprise = derniereReponse + DELAI_NOUVELLE_PROPOSITION_JOURS * 24 * 60 * 60 * 1000;
  return Date.now() < reprise;
}

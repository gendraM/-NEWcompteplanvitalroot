...
export async function appliquerDecisionValidation(defi, decision, options = {}) {
  ...
  // Les défis DURATION avancent uniquement avec le temps. Leurs observations
  // alimentent le feedback/bilan mais ne doivent jamais appeler la RPC OCCURRENCES.
  if (defi?.progression_model === 'duration' || decision.donnees?.observationSeulement === true) {
    return {
      success: true,
      progressionIncrementee: false,
      etapeValidee: false,
      observationEnregistree: false,
      decision: decision.decision
    };
  }

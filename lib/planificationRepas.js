...
export function construireComposantAssiette(aliment, quantite, unite = null, id = null) {
  ...
  return {
    ...
    composant: {
      ...
      profilAlimentaire: aliment.profilAlimentaire || null,
...
export function construireAssietteDepuisRepere(referentiel = [], repere = {}) {
  const composition = Array.isArray(repere?.composition) ? repere.composition : [];
  if (composition.length < 2) {
    return { erreur: 'Cette assiette repère ne contient pas assez d’aliments pour être chargée.', composition: [] };
  }

  const composants = [];
  for (const [index, ligne] of composition.entries()) {
    const reference = trouverAlimentReferentiel(referentiel, ligne?.aliment || ligne?.nom);
    if (!reference) {
      return {
        erreur: `L’aliment « ${ligne?.aliment || ligne?.nom || 'inconnu'} » n’est plus disponible dans ton référentiel.`,
        composition: []
      };
    }

    const mesure = extraireQuantitePlanifiee(ligne?.quantite);
    const quantiteNumerique = mesure.quantite ?? nombreFini(ligne?.quantite);
    const unite = mesure.unite || reference.unite;
    if (quantiteNumerique === null || !unite) {
      return {
        erreur: `La quantité de « ${reference.nom} » doit être complétée avant de pouvoir réutiliser cette assiette.`,
        composition: []
      };
    }

    const resultat = construireComposantAssiette(
      reference,
      quantiteNumerique,
      unite,
      `repere-${normaliserNomAliment(reference.nom)}-${index}`
    );
    if (resultat.erreur) return { erreur: resultat.erreur, composition: [] };
    composants.push(resultat.composant);
  }

  return { erreur: null, composition: composants };
}
...
export async function deplacerRepasPlanifie(supabase, lignes = [], nouvelleDate, userId) {
  const ids = [...new Set((Array.isArray(lignes) ? lignes : []).map(ligne => ligne?.id).filter(Boolean))];
  if (!supabase?.from || !userId || !/^\d{4}-\d{2}-\d{2}$/.test(String(nouvelleDate || '')) || ids.length === 0) {
    return { data: null, error: new Error('Le repas ou sa nouvelle date est invalide.') };
  }

  const { data, error } = await supabase
    .from('repas_planifies')
    .update({ date: nouvelleDate })
    .in('id', ids)
    .eq('user_id', userId)
    .select('*');
  ...
  if (!Array.isArray(data) || data.length !== ids.length) {
    return { data: data || [], error: new Error('Toutes les lignes du repas n’ont pas pu être déplacées.') };
  }
...
export function grouperRepasPlanifiesParType(repasPlanifies = []) {
  return (repasPlanifies || []).reduce((groupes, repas) => {
    if (!repas?.type) return groupes;
    if (!groupes[repas.type]) groupes[repas.type] = [];
    groupes[repas.type].push(repas);
    return groupes;
  }, {});
}

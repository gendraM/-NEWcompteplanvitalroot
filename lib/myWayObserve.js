...
export function buildMealObservation(rows = [], periodDays = 28) {
  ...
  return {
    id: 'meals-28d', family: 'alimentation', evidenceLevel: 'P1', periodDays,
...
export function buildWeightObservation(rows = []) {
  const valid = rows.map((row) => ({ date: row?.date, poids: Number(row?.poids) }))
  ...
  return {
    id: 'weight-history', family: 'corps', evidenceLevel: 'P1', sourceTables: ['historique_poids'],
...
export function buildIdeauxObservations(ideaux = [], sessions = [], now = new Date()) {
  ...
  return [...grouped.entries()].filter(([, counts]) => counts.total >= 2)
    .sort((a, b) => b[1].total - a[1].total).slice(0, 2)
    .map(([idealId, counts], index) => ({
      id: `ideal-sessions-${index}`, family: 'engagement', evidenceLevel: 'P1', sourceTables: ['ideaux', 'seances_reelles'],
...
function recoveryEpisodes(sessions) {
  const episodes = []; let missed = 0;
  for (const session of sessions) {
    if (session.fait === true) { if (missed > 0) episodes.push(missed); missed = 0; } else missed += 1;
  }
  return episodes;
}
...
export function buildIdeauxRecoveryTrends(ideaux = [], sessions = [], now = new Date()) {
  const today = now.toISOString().slice(0, 10);
  const recentStart = isoDateDaysAgo(27, now); const previousStart = isoDateDaysAgo(55, now);
  const previousEndDate = new Date(now); previousEndDate.setHours(0, 0, 0, 0); previousEndDate.setDate(previousEndDate.getDate() - 28);
  const previousEnd = previousEndDate.toISOString().slice(0, 10);
  const titles = new Map(ideaux.map((ideal) => [String(ideal.id), String(ideal.titre || 'Cet Idéal').trim()]));
  const trends = [];
  for (const [idealId, title] of titles.entries()) {
    const eligible = sessions.filter((session) => String(session?.ideal_id || '') === idealId)
      .map((session) => ({ date: String(session?.date_prevue || session?.date || '').slice(0, 10), fait: session?.fait === true }))
      .filter((session) => session.date && session.date <= today && session.date >= previousStart).sort((a, b) => a.date.localeCompare(b.date));
    const previousEpisodes = recoveryEpisodes(eligible.filter((session) => session.date >= previousStart && session.date <= previousEnd));
    const recentEpisodes = recoveryEpisodes(eligible.filter((session) => session.date >= recentStart && session.date <= today));
    ...
    const delta = Number((recentAverage - previousAverage).toFixed(2)); const absoluteDelta = Math.abs(delta);
    ...
    trends.push({
      id: `ideal-recovery-${idealId}`, kind: 'recovery_after_interruption', family: 'engagement', evidenceLevel: 'P3',
      sourceTables: ['ideaux', 'seances_reelles'], idealId, direction,
      text: `Pour « ${title} », le nombre moyen de séances non réalisées avant une reprise ${direction === 'faster' ? 's’est raccourci' : 's’est allongé'} sur la période récente par rapport à la période précédente.`,
      metrics: { previousEpisodeCount: previousEpisodes.length, recentEpisodeCount: recentEpisodes.length, previousAverageMissedBeforeRecovery: Number(previousAverage.toFixed(2)), recentAverageMissedBeforeRecovery: Number(recentAverage.toFixed(2)), delta, relativeChange, previousPeriod: { start: previousStart, end: previousEnd }, recentPeriod: { start: recentStart, end: today } },
    });
  }
  return trends;
}
...
export function buildIdeauxWeightTrends(ideaux = [], sessions = [], weights = [], now = new Date()) {
  ...
  const previousEndDate = new Date(now); previousEndDate.setHours(0, 0, 0, 0); previousEndDate.setDate(previousEndDate.getDate() - 28);
  const previousEnd = previousEndDate.toISOString().slice(0, 10);
  const validWeights = weights.map((row) => ({ date: String(row?.date || '').slice(0, 10), poids: Number(row?.poids) }))
  ...
  for (const ideal of ideaux) {
    const idealId = String(ideal?.id || ''); if (!idealId) continue;
    const eligible = sessions.filter((session) => String(session?.ideal_id || '') === idealId)
      .map((session) => ({ date: String(session?.date_prevue || session?.date || '').slice(0, 10), fait: session?.fait === true }))
    ...
    trends.push({
      id: `ideal-weight-${idealId}`, kind: 'ideal_weight_coevolution', family: 'cross_module', evidenceLevel: 'P3',
      sourceTables: ['ideaux', 'seances_reelles', 'historique_poids'], idealId,
      ...
      metrics: {
        previousSessionCount: previousSessions.length, recentSessionCount: recentSessions.length,
        previousCompletionRate: Number(previousRate.toFixed(4)), recentCompletionRate: Number(recentRate.toFixed(4)), completionRateDelta,
        previousWeightMeasurementCount: previousWeights.length, recentWeightMeasurementCount: recentWeights.length,
        previousMedianWeight: Number(previousMedianWeight.toFixed(2)), recentMedianWeight: Number(recentMedianWeight.toFixed(2)), weightDelta,
        previousPeriod: { start: previousStart, end: previousEnd }, recentPeriod: { start: recentStart, end: today },
      },
    });
  }
  return trends;
...
async function safeQuery(label, queryPromise) {
  try { const { data, error } = await queryPromise; if (error) return { label, data: [], available: false }; return { label, data: data || [], available: true }; }
  catch { return { label, data: [], available: false }; }
}
...
export async function collectMyWayObservations(client, userId, now = new Date()) {
  ...
  const mealsSince = isoDateDaysAgo(27, now); const sessionsSince = isoDateDaysAgo(55, now);
  ...
  const observations = [];
  if (mealsResult.available) { const fact = buildMealObservation(mealsResult.data, 28); if (fact) observations.push(fact); }
  if (weightResult.available) { const fact = buildWeightObservation(weightResult.data); if (fact) observations.push(fact); }
  if (idealsResult.available && sessionsResult.available) observations.push(...buildIdeauxObservations(idealsResult.data, sessionsResult.data, now));
  return { status: observations.length ? 'FACTS' : 'NO_INTERVENTION', observations: observations.slice(0, 4), sources: { repas: mealsResult.available, poids: weightResult.available, ideaux: idealsResult.available && sessionsResult.available } };
}

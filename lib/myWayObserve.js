...
export function buildIdeauxWeightTrends(ideaux = [], sessions = [], weights = [], now = new Date()) {
  ...
  const hasSufficientWeightCoverage = (periodWeights) => {
    const dates = [...new Set(periodWeights.map((row) => row.date))].sort();
    if (dates.length < 2) return false;
    const firstDate = new Date(`${dates[0]}T00:00:00Z`);
    const lastDate = new Date(`${dates[dates.length - 1]}T00:00:00Z`);
    return (lastDate - firstDate) / DAY_MS >= 7;
  };
  if (!hasSufficientWeightCoverage(previousWeights) || !hasSufficientWeightCoverage(recentWeights)) return [];
...
export function selectObserveIntervention(candidates = [], previousInterventions = []) {
  const eligible = candidates.filter((candidate) =>
    candidate
    && candidate.evidenceLevel !== 'P1'
    && ['recovery_after_interruption', 'ideal_weight_coevolution'].includes(candidate.kind)
  );

  if (!eligible.length) {
    return { status: 'NO_INTERVENTION', observation: null };
  }

  const seen = new Set(
    previousInterventions
      .map((item) => item?.observationId || item?.id)
      .filter(Boolean)
      .map(String)
  );
  const unseen = eligible.filter((candidate) => !seen.has(String(candidate.id)));
  ...
  const priority = {
    ideal_weight_coevolution: 30,
    recovery_after_interruption: 20,
  };
  const ranked = unseen
    .map((candidate, index) => ({ candidate, index }))
    .sort((a, b) => (priority[b.candidate.kind] || 0) - (priority[a.candidate.kind] || 0) || a.index - b.index);

  return {
    status: 'INTERVENTION',
    observation: ranked[0].candidate,
  };
}

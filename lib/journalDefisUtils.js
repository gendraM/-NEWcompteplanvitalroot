...
export async function validerEtapeDefi(defiId, jour, engagements) {
  try {
    ...
    const etapeValidee = calculerScoreNumerique(engagements) >= (2 / 3);
    const { data: journal, error: journalSelectError } = await supabase.from('journal_defis').select('id, valide').eq('defi_id', defiId).eq('jour', jour).eq('user_id', userId).maybeSingle();
    ...
    const { error: journalError } = await supabase.from('journal_defis').update({ engagements, score, valide: etapeValidee, updated_at: new Date().toISOString() }).eq('id', journal.id).eq('user_id', userId);
    ...
    const { data: defi, error: defiError } = await supabase.from('defis').select('*').eq('id', defiId).eq('user_id', userId).single();
    ...
    return { ...progression, etapeValidee: true, progressionIncrementee: progression.progressionIncrementee === true, newProgress: progression.newProgress ?? progression.nouvelleProgression ?? defi.progress };
  } catch (err) { return { success: false, etapeValidee: false, error: err.message }; }
}

// Compatibilité transitoire : l'UI historique utilise `valide`, le contrat cible utilise `tenu`.
function engagementTenu(engagement) {
  return engagement?.tenu === true || engagement?.valide === true;
}

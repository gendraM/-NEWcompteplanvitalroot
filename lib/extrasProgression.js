...
export function calculerMeilleureConstanceExtras(semaines = [], palier, depuis = null) {
  const palierNumerique = lireNombre(palier);
  if (![1, 2, 3, 5].includes(palierNumerique)) return null;

  const semainesTriees = [...(Array.isArray(semaines) ? semaines : [])]
    .filter(semaine => {
      const date = lireDateSemaine(semaine);
      return date && (!depuis || date >= depuis);
    })
  ...
  for (const semaine of semainesTriees) {
    const date = lireDateSemaine(semaine);
    const evaluation = evaluerSemaineExtras(semaine, palierNumerique);
    const consecutive = datePrecedente !== null && ecartEnSemaines(datePrecedente, date) === 1;

    if (!evaluation.comptePourProgression) {
      serie = [];
    } else {
      serie = consecutive ? [...serie, date] : [date];
      if (serie.length > meilleure.length) meilleure = [...serie];
    }
    datePrecedente = date;
  }

  if (!meilleure.length) return null;
  return {
    nombreSemaines: meilleure.length,
    debut: meilleure[0],
    fin: meilleure[meilleure.length - 1],
  };
}

...
export function verifierHeureRepas(repas7j = []) {
  ...
  function convertirHeureEnMinutes(heureSource) {
    if (!heureSource) return null;
    const texte = String(heureSource).trim();
    const match = texte.match(/^(\d{1,2}):(\d{1,2})$/);
  ...
  Object.keys(parDate).forEach(date => {
    ...
    repasJour.forEach(repas => {
      ...
      const minutes = convertirHeureEnMinutes(heureSource);
      if (Number.isFinite(minutes)) {
        if (derniereMinute === null || minutes > derniereMinute) {
          derniereMinute = minutes;
        }
      }
    });
    
    // Vérifier si dernier repas strictement avant 19:00
    if (derniereMinute !== null && derniereMinute < (19 * 60)) {
      joursOK++;
    }
  });
...
export function calculerDureeRepas(repas7j = []) {
  ...
  const parDateEtType = {};
  repas7j.forEach(repas => {
    const cle = `${repas.date}_${repas.type || repas.type_repas || 'Repas'}`;
  ...
  Object.keys(parDateEtType).forEach(cle => {
    ...
    if (!statutParDate[date]) {
      statutParDate[date] = { analysable: false, nonConforme: false };
    }

    const heuresMinutes = repas
      .map(item => convertirHeureEnMinutes(item.heureRepas || item.heure_repas || item.heure))
      .filter(Number.isFinite)
      .sort((a, b) => a - b);

    if (heuresMinutes.length === 0) {
      return;
    }

    statutParDate[date].analysable = true;

    if (heuresMinutes.length === 1) {
      return;
    }

    const dureeMinutes = heuresMinutes[heuresMinutes.length - 1] - heuresMinutes[0];
    if (dureeMinutes > 45) {
      statutParDate[date].nonConforme = true;
    }
  });

  let joursOK = 0;
  Object.values(statutParDate).forEach(statut => {
    if (statut.analysable && !statut.nonConforme) {
      joursOK++;
    }
  });
...
export function getCritereIdFromLabel(label) {
  ...
  const mapping = {
    ...
    'Phase 3 Pré-jeûne : hydratation, horaire, durée': 7,

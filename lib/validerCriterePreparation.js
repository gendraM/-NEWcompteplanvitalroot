...
export function getStatutCritereAuto(critereId, repas7j = []) {
  ...
  switch (critereId) {
    case 1:
      joursRespectés = analyserPortions(repas7j, arguments[2] || []);
...
export function analyserPortions(repas7j = [], referentielAliments = []) {
  ...
  Object.keys(parDate).forEach(date => {
    ...
    // Si ≥ 80% des repas analysables du jour ont des portions correctes
    if (totalRepasAnalysables > 0 && (portionsCorrectes / totalRepasAnalysables) >= 0.8) {
      joursOK++;
    }
  });

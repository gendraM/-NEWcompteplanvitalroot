...
function phaseSurPeriode(programme, phase, limiteJours = 7, debutJours = 1) {
  const configuration = programme?.phases?.[`phase${phase}`];
  if (!configuration) return null;
  const debut = Math.max(1, debutJours, Number(configuration.debut));
...
export function creerConfigurationCoursesReprise(programme, limiteJours = 7, debutJours = 1) {
  ...
  Object.entries(CONFIGURATION_COURSES_PAR_PHASE).forEach(([phaseTexte, configuration]) => {
    ...
    const periode = phaseSurPeriode(programme, phase, periodeFin, debutJours);
    if (!periode) return;
    ...
    configuration.groupes.forEach(groupe => {
      const debutEligible = Math.max(
        periode.debut,
        Number(programme.phases[`phase${phase}`].debut) + Number(groupe.jourPhaseMin || 1) - 1
      );
  ...
  return {
    periode: { debut: debutJours, fin: periodeFin, libelle: `J${debutJours} à J${periodeFin}` },
    indispensables,
    groupes
  };
}
...
export function genererListeCoursesPersonnalisee(programme, choix = {}, limiteJours = 7, debutJours = 1) {
  const configuration = creerConfigurationCoursesReprise(programme, limiteJours, debutJours);
...
function normaliserItem(item, categorie = CATEGORIE_PAR_DEFAUT) {
  ...
  return {
    ...
    article_id: item.article_id || creerIdentifiantArticle(item),
    statut_achat: ['a_acheter', 'achete', 'deja_disponible'].includes(item.statut_achat)
      ? item.statut_achat
      : 'a_acheter',
    statut_modifie_le: item.statut_modifie_le || null
  };
}
...
export function creerIdentifiantArticle(item) {
  return [
    item.phase || (item.phases || []).join('-'),
    item.groupe_id || item.type || 'article',
    item.nom,
    item.preparation
  ].map(nettoyerIdentifiant).filter(Boolean).join('__');
}

export function initialiserEtatsListeCourses(listeCourses) {
  return normaliserListeCoursesReprise(listeCourses).map(item => ({
    ...item,
    article_id: item.article_id || creerIdentifiantArticle(item),
    statut_achat: item.statut_achat || 'a_acheter'
  }));
}

export function modifierStatutArticle(listeCourses, articleId, statut) {
  if (!['a_acheter', 'achete', 'deja_disponible'].includes(statut)) return initialiserEtatsListeCourses(listeCourses);
  return initialiserEtatsListeCourses(listeCourses).map(item =>
    item.article_id === articleId
      ? { ...item, statut_achat: statut, statut_modifie_le: new Date().toISOString() }
      : item
  );
}

export function alternativesArticle(programme, article, limiteJours = 7, debutJours = 1) {
  if (!article?.groupe_id || article.type === 'indispensable') return [];
  const configuration = creerConfigurationCoursesReprise(programme, limiteJours, debutJours);
  const groupe = configuration.groupes.find(item => item.id === article.groupe_id);
  return (groupe?.options || []).filter(option => option.nom !== article.nom);
}

export function remplacerArticleCourses(programme, listeCourses, articleId, nouveauNom, choixCourses = {}, limiteJours = 7, debutJours = 1) {
  const listeActuelle = initialiserEtatsListeCourses(listeCourses);
  const article = listeActuelle.find(item => item.article_id === articleId);
  if (!article?.groupe_id || article.type === 'indispensable') {
    return { liste: listeActuelle, choix: choixCourses, remplace: false };
  }
  const alternatives = alternativesArticle(programme, article, limiteJours, debutJours);
  if (!alternatives.some(option => option.nom === nouveauNom)) {
    return { liste: listeActuelle, choix: choixCourses, remplace: false };
  }

  const choixCompletes = listeActuelle.reduce((acc, item) => {
    if (item.groupe_id && !acc[item.groupe_id]) acc[item.groupe_id] = [];
    if (item.groupe_id && !acc[item.groupe_id].includes(item.nom)) acc[item.groupe_id].push(item.nom);
    return acc;
  }, { ...choixCourses });
  const selectionActuelle = choixCompletes[article.groupe_id] || [];
  const prochaineSelection = Array.from(new Set(
    selectionActuelle.map(nom => nom === article.nom ? nouveauNom : nom)
  ));
  if (!prochaineSelection.includes(nouveauNom)) prochaineSelection.push(nouveauNom);
  const prochainsChoix = { ...choixCompletes, [article.groupe_id]: prochaineSelection };
  const regeneree = initialiserEtatsListeCourses(
    genererListeCoursesPersonnalisee(programme, prochainsChoix, limiteJours, debutJours)
  );
  const statutsExistants = new Map(listeActuelle.map(item => [item.article_id, item]));
  return {
    liste: regeneree.map(item => {
      const ancien = statutsExistants.get(item.article_id);
      return ancien ? { ...item, statut_achat: ancien.statut_achat, statut_modifie_le: ancien.statut_modifie_le } : item;
    }),
    choix: prochainsChoix,
    remplace: true
  };
}

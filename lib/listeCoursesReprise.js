...
function formaterQuantite(valeur, unite) {
  const arrondi = unite === 'unité' ? Math.max(1, Math.ceil(valeur)) : Math.ceil(valeur / 10) * 10;
  return `${arrondi} ${unite}${unite === 'unité' && arrondi > 1 ? 's' : ''}`;
}

function construireArticle(option, phase, jours, type, groupeId = null, nombreOptions = 1) {
  const joursRepartis = Math.max(1, Math.ceil(jours / Math.max(1, nombreOptions)));
  const quantite = option.quantiteFixe || formaterQuantite(option.quantiteParJour * joursRepartis, option.unite);
  return {
    nom: option.nom,
    quantite,
    ...
    utilisations_estimees: joursRepartis
  };
}
...
export function genererListeCoursesPersonnalisee(programme, choix = {}, limiteJours = 7) {
  ...
  return [...configuration.indispensables, ...articlesChoisis].sort((a, b) =>

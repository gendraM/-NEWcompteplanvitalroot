...
import { estRepasExtra } from './validationSemaine';
...
export async function calculerSection2BudgetCalorique(mois, annee, objectifCaloriqueJour = 1900) {
  try {
    ...
    const repasExtras = repas.filter(estRepasExtra);
...
export async function calculerSection3Patterns(mois, annee, objectifCaloriqueJour = 1900) {
  try {
    ...
    repas.forEach(r => {
      ...
      if (estRepasExtra(r)) repasByDate[r.date].extras++;
    ...
    const extras_total = repas.filter(estRepasExtra).length;
...
export async function calculerSection5BienEtre(mois, annee) {
  try {
    ...
    depassements.forEach(r => {
      ...
      // Extras
      if (estRepasExtra(r)) analyse_depassements.avec_extras++;
    ...
    humeurNegative.forEach(r => {
      ...
      // Extras
      if (estRepasExtra(r)) analyse_humeur_negative.avec_extras++;
 ...
 */
export async function genererBilanCompletMensuel(mois, annee) {
  try {
    console.log('[CALCUL BILAN] Génération bilan complet', { mois, annee });
    ...
    const nb_extras_total = repasDetails?.filter(estRepasExtra).length || 0;
    
    // Préparer l'objet bilan EXACTEMENT comme bilan hebdo (PAS de user_id)
    const bilanToInsert = {
      mois,
      annee,
    ...
    console.log('[CALCUL BILAN] Objet à enregistrer (upsert):', {
      mois,
      annee,
      nbJoursTotal: nb_jours_total,
      nbJoursSaisis: nb_jours_saisis
    });
    
    // Utiliser upsert EXACTEMENT comme bilan hebdo
    const { data: bilan, error } = await supabase
      .from('bilans_mensuels')
      .upsert([bilanToInsert], {
        onConflict: 'mois,annee',  // Clé unique comme weekStart pour bilan hebdo
        ignoreDuplicates: false
      });
    
    if (error) {
      console.error('[CALCUL BILAN] Erreur upsert:', error);
      console.error('[CALCUL BILAN] Code:', error.code, 'Message:', error.message);
      return null;
    }
    
    console.log('[CALCUL BILAN] ✅ Bilan généré et sauvegardé avec succès');
    return bilan || { success: true };
  } catch (err) {

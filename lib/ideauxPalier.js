...
export function calculerProgressionPalier(semaines, seancesReelles = []) {
  const datesPrevues = new Set(
    (semaines || []).flatMap((semaine) =>
      (semaine.actions || []).map((action) => action.date)
    )
  );

  const total = datesPrevues.size;
  const faites = (seancesReelles || []).filter(
    (seance) => !seance.bonus && datesPrevues.has(seance.date_prevue) && seanceEstFaite(seance)
  ).length;

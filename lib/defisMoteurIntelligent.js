...
export function construireCandidatsDefis({ signaux = [], defisDisponibles = [] } = {}) {
  ...
  for (const signal of signaux) {
    ...
    if (!defi || dejaAjoutes.has(defi.id)) continue;
    dejaAjoutes.add(defi.id);
    candidats.push({
      defi,
      signal: signal.type,
      score: signal.score,
...
export function choisirPropositionDefi({
  ...
  const exclus = new Set(defisExclus.map(Number));
  const proposition = construireCandidatsDefis({ signaux, defisDisponibles })
    .find(candidat => !exclus.has(Number(candidat.defi.id)));

  if (proposition) {
    return { proposition, raisonBlocage: null };
  }

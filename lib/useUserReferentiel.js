...
import { enrichirReferentielAvecProfils } from '../data/profilsAlimentaires';
import { supabase } from './supabaseClient';

/**
 * Hook pour fusionner le référentiel global et utilisateur
 * Retourne les listes séparées et fusionnées
 * Expose une fonction refresh() pour forcer un rechargement des aliments custom
 */
export default function useUserReferentiel(userId) {
  ...
  useEffect(() => {
    setReferentielGlobal(enrichirReferentielAvecProfils(foodsGlobal));
  }, []);
  ...
  useEffect(() => {
    const fusion = enrichirReferentielAvecProfils([...referentielGlobal, ...referentielCustom]);
  ...
  return { referentielGlobal, referentielCustom, referentielComplet, loading, refresh };
}

import { PHASES_REPRISE, getAlimentsDisponiblesPhase } from './repriseJeuneMetier';
...
export function genererProgrammeReprise({ dureeJeune, poidsDepart, dateFin, options = {} }) {
  ...
  for (let jourNum = 1; jourNum <= dureeReprise; jourNum++) {
    ...
    const jourDansPhase = jourNum - phaseConfig.debut + 1;
    const alimentsAutorises = getAlimentsDisponiblesPhase(phase, jourDansPhase);
    ...
    joursDetailles.push({
      ...
      aliments_autorises: alimentsAutorises.map(a => ({
        ...
        phase_introduction: a.phase,
        nouveau_dans_phase: a.phase === phase
      })),
  ...
  const programme = {
    ...
    phases: {
      phase1: {
        nom: PHASES_REPRISE[1].nom,
        debut: phases.phase1.debut,
        fin: phases.phase1.fin,
        objectif: PHASES_REPRISE[1].objectif,
        qn_minimum: PHASES_REPRISE[1].qnMinimum
      },
      phase2: {
        nom: PHASES_REPRISE[2].nom,
        debut: phases.phase2.debut,
        fin: phases.phase2.fin,
        objectif: PHASES_REPRISE[2].objectif,
        qn_minimum: PHASES_REPRISE[2].qnMinimum
      },
      phase3: {
        nom: PHASES_REPRISE[3].nom,
        debut: phases.phase3.debut,
        fin: phases.phase3.fin,
        objectif: PHASES_REPRISE[3].objectif,
        qn_minimum: PHASES_REPRISE[3].qnMinimum
      },
      phase4: {
        nom: PHASES_REPRISE[4].nom,
        debut: phases.phase4.debut,
        fin: phases.phase4.fin,
        objectif: PHASES_REPRISE[4].objectif,
        qn_minimum: PHASES_REPRISE[4].qnMinimum
      },
      phase5: {
        nom: PHASES_REPRISE[5].nom,
        debut: phases.phase5.debut,
        fin: phases.phase5.fin,
        objectif: PHASES_REPRISE[5].objectif,
        qn_minimum: PHASES_REPRISE[5].qnMinimum
      }
    },
...
export function getAlimentsPhase(phase) {
  return getAlimentsDisponiblesPhase(phase);
}

...
import { getValidationConfig } from './defisValidationReferentiel';
...
export async function initDefisUser() {
  ...
  for (const defi of defisReferentiel) {
    ...
    const { error: errorInsert } = await supabase.from('defis').insert({
      ...
      duree: defi.duree,
      unite: defi.unite,
      progression_model: validationConfig.progressionModel,
      duree_unite: validationConfig.dureeUnite || null,
      progress: 0,

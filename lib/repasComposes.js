...
export function genererOccurrenceRepasId() {
  if (typeof globalThis !== 'undefined' && globalThis.crypto?.randomUUID) {
    return globalThis.crypto.randomUUID();
  }

  // Fallback RFC 4122 v4 pour les environnements de test anciens sans crypto.randomUUID.
...
export function normaliserComposantRepas(composant, index = 0) {
  ...
  return {
    ...
    profilAlimentaire: composant?.profilAlimentaire || null,
    quantite,
...
export function ajusterCompositionRepas(composition = [], quantites = []) {
  const composantsOrigine = composition.map(normaliserComposantRepas);
  const erreurs = [];
  if (composantsOrigine.length < 2) {
    erreurs.push('Un repas composé doit contenir au moins deux aliments.');
  }
  const composants = composantsOrigine.map((composant, index) => {
    const nouvelleQuantite = nombreFini(quantites[index]);
    if (nouvelleQuantite === null || nouvelleQuantite <= 0) {
      erreurs.push(`La quantité de ${composant.nom || `l’aliment ${index + 1}`} doit être supérieure à 0.`);
      return { ...composant, quantite: nouvelleQuantite, kcal: null };
    }
    if (composant.quantite === null || composant.quantite <= 0 || composant.kcal === null) {
      erreurs.push(`La quantité de référence ou les calories de ${composant.nom || `l’aliment ${index + 1}`} sont indisponibles.`);
      return { ...composant, quantite: nouvelleQuantite, kcal: null };
    }
    ...
    return {
      ...
      kcal: Math.round(composant.kcal * nouvelleQuantite / composant.quantite)
    };
  });

  return {
    valide: erreurs.length === 0 && composants.length >= 2,
    erreurs,
    composition: composants,
    resume: calculerResumeRepasCompose(composants)
  };
}
...
export function construireOccurrencesReelles(
  repas,
  { userId, date, type, heure = null, satiete = '', ressenti = '', note = '', occurrenceRepasId = null }
) {
  const modele = normaliserRepasCompose(repas);
  if (!userId || !date || !type || !validerCompositionRepas(modele.composition).valide) return [];

...
// Les outils de simulation restent disponibles en développement, jamais en production.
export const outilsModeTestDisponibles = () => process.env.NODE_ENV !== 'production';
...
export const initialiserDateModeTest = () => {
  if (!outilsModeTestDisponibles() || typeof window === 'undefined') return null;
...
export const avancerDateModeTest = () => {
  if (!outilsModeTestDisponibles()) return null;
  ...
  if (typeof window.dispatchEvent === 'function' && typeof Event !== 'undefined') {
    window.dispatchEvent(new Event('mode-test-date-change'));
  }
  return suivante;
};

export const positionnerDateModeTestMinimum = valeur => {
  if (!estModeTestActif() || !valeur) return null;
  const actuelle = initialiserDateModeTest();
  if (actuelle >= valeur) return actuelle;
  localStorage.setItem(MODE_TEST_DATE_KEY, valeur);
  if (typeof window.dispatchEvent === 'function' && typeof Event !== 'undefined') {
    window.dispatchEvent(new Event('mode-test-date-change'));
  }
